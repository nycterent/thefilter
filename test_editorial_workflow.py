...
async def demo_editorial_workflow():
    ...
    class MockOpenRouterClient:
        ...
        async def revise_content(self, original_content, editor_feedback, article_content="", user_highlights=""):
...
"""
    ...
    if newsletter_review["approved"]:
        print(f"✅ Newsletter approved by editor (score: {newsletter_review['score']}/10)")
    else:
        print(f"⚠️ Newsletter needs improvement (score: {newsletter_review['score']}/10)")
    ...
    avg_score = sum(editorial_stats['editor_scores']) / len(editorial_stats['editor_scores']) if editorial_stats['editor_scores'] else 0
    print(f"   - Average editor score: {avg_score:.1f}/10")
    print(f"   - Newsletter editor score: {editorial_stats['newsletter_editor_score']}/10")
