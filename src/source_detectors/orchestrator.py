...
class SourceDetectionOrchestrator:
    ...
    def register_detector(self, detector: SourceDetector):
        ...
        if existing:
            logger.warning(
                f"Replacing existing detector for provider: {detector.provider_name}"
            )
        ...
        logger.info(
            f"Registered detector: {detector.provider_name} (priority: {detector.get_priority()})"
        )
    ...
    async def detect_source(self, url: str) -> Optional[SourceDetectionResult]:
        ...
        for detector in self.detectors:
            try:
                if detector.is_applicable(url):
                    ...
                    logger.debug(
                        f"❌ Detector {detector.provider_name} is not applicable"
                    )
            except Exception as e:
                logger.error(
                    f"Error checking applicability for {detector.provider_name}: {e}"
                )
                continue
        ...
        logger.info(
            f"Found {len(applicable_detectors)} applicable detector(s): {[d.provider_name for d in applicable_detectors]}"
        )
            ...
            try:
                ...
                if result:
                    if result.status == DetectionStatus.SUCCESS:
                        logger.info(
                            f"✅ Successful detection by {detector.provider_name}"
                        )
                        return result
                    elif result.status == DetectionStatus.PARTIAL_SUCCESS:
                        logger.warning(
                            f"⚠️ Partial success with {detector.provider_name}"
                        )
                        return result
                    else:
                        logger.warning(
                            f"❌ Detection failed with {detector.provider_name}: {result.error_message}"
                        )
    ...
    async def detect_source_with_fallback(
        self, url: str, fallback_provider: Optional[str] = None
