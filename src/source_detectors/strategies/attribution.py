...
class DomainExtractionStrategy(AttributionStrategy):
    ...
    def extract(self, content: str) -> Optional[AttributionInfo]:
        ...
        excluded_domains = {
            'mailchimp.com', 'campaign-archive.com', 'list-manage.com',
            'facebook.com', 'twitter.com', 'linkedin.com', 'instagram.com',
            'youtube.com', 'google.com', 'tracking.com', 'utm.com',
            'feedburner.com', 'bit.ly', 'tinyurl.com'
        }
        ...
        for strategy in self.strategies:
            try:
                ...
                if result and result.confidence_score > best_score:
                    ...
                    logger.debug(f"Better attribution found using {strategy.strategy_name}: {result.publisher} (score: {result.confidence_score})")
            except Exception as e:
                logger.warning(f"Error in strategy {strategy.strategy_name}: {e}")
                continue
        
        if best_result:
            logger.info(f"Best attribution: {best_result.publisher} using {best_result.extraction_method} (confidence: {best_result.confidence_score})")
