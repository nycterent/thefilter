...
class AttributionInfo(BaseModel):
    ...
    confidence_score: float = Field(
        ...
        ge=0.0, 
        le=1.0, 
        description="Confidence in attribution (0.0 to 1.0)"
    )
    extraction_method: Optional[str] = Field(
        None,
        description="Method used to extract attribution"
    )
...
class SourceDetectionResult(BaseModel):
    ...
    provider: str = Field(
        ..., 
        description="Name of the source detector provider"
    )
    url: str = Field(
        ..., 
    ...
    status: DetectionStatus = Field(
        ..., 
        description="Status of the detection process"
    )
    content_extracted: bool = Field(
        False, 
        description="Whether content was successfully extracted"
    )
    attribution_found: bool = Field(
        False, 
        description="Whether attribution was found"
    )
