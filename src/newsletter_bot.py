...
def generate(ctx: click.Context, dry_run: bool) -> None:
    ...
    async def _generate():
        try:
            ...
            if missing_keys:
                logger.error(f"Missing required API keys for: {', '.join(missing_keys)}")
                logger.error("Please configure secrets in Infisical or environment variables")
                return
            ...
            failed_connections = [service for service, status in connections.items() 
                                if not status and service != 'rss_feeds']
            
            if failed_connections:
                logger.warning(f"⚠️  Some services are unavailable: {', '.join(failed_connections)}")
            ...
            if newsletter.metadata:
                if newsletter.metadata.get('readwise_items'):
                    logger.info(f"   - Readwise highlights: {newsletter.metadata['readwise_items']}")
                if newsletter.metadata.get('rss_items'):
                    logger.info(f"   - RSS articles: {newsletter.metadata['rss_items']}")
...
def health() -> None:
    ...
    try:
            ...
            for service, status in connections.items():
                if service == 'rss_feeds':
                    for feed_url, feed_status in status.items():
                        ...
                        logger.info(f"   - RSS Feed ({feed_url[:50]}...): {status_icon}")
                else:
        ...
        if critical_missing:
            logger.warning(f"⚠️  Missing critical API keys: {', '.join(critical_missing)}")
...
def config() -> None:
    ...
    try:
        ...
        keys_status = {
            ...
            'Buttondown': '✅ Configured' if settings.buttondown_api_key else '❌ Missing',
