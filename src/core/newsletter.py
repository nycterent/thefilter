...
class NewsletterGenerator:
    ...
    def __init__(self, settings: Settings):
        ...
        self.readwise_client = ReadwiseClient(settings.readwise_api_key) if settings.readwise_api_key else None
        
        # Parse RSS feeds from comma-separated string
        rss_feeds = []
        if settings.rss_feeds:
            rss_feeds = [url.strip() for url in settings.rss_feeds.split(',') if url.strip()]
    ...
    async def generate_newsletter(self, dry_run: bool = False) -> NewsletterDraft:
        ...
        if not dry_run and self.settings.buttondown_api_key:
            ...
            logger.info("Newsletter generation completed (not published - dry run mode)")
    ...
    async def _aggregate_content(self) -> List[ContentItem]:
        ...
        logger.info(f"Collected {len(all_content)} items, {len(unique_content)} after deduplication")
    ...
    async def _process_content(self, content_items: List[ContentItem]) -> List[ContentItem]:
    ...
    async def _create_newsletter_draft(self, content_items: List[ContentItem]) -> NewsletterDraft:
