...
class VoiceManager:
    ...
    def get_voice_generator(self, voice_name: Optional[str] = None) -> VoiceGenerator:
        ...
        if voice_name not in self._generators:
            ...
            raise ValueError(
                f"Voice '{voice_name}' not available. Available: {available}"
            )
    ...
    async def generate_commentary(
        ...
        try:
            ...
            logger.info(
                f"Generated commentary using voice '{voice or self.default_voice}'"
            )
    ...
    async def generate_multi_language(
        ...
        if not valid_languages:
            logger.warning(
                f"No valid languages found. Supported by voice: {supported_languages}"
            )
        ...
        for language in valid_languages:
            try:
                ...
                logger.error(
                    f"Failed to generate commentary for language '{language}': {e}"
                )
