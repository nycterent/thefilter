...
def check_truncation(text: str) -> Dict[str, any]:
    ...
    single_quotes = len(re.findall(r"(?<!\w)'", text))  # Single quotes not preceded by word char
    double_quotes = len(re.findall(r'"(?=[^"]*[a-zA-Z])', text))  # Double quotes followed by text
...
def main():
    ...
    if "--json" in sys.argv:
        ...
        print(f"Checks: {results['summary']['passed_checks']}/{results['summary']['total_checks']} passed")
        print()
