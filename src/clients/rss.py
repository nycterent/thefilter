...
class RSSClient:
    ...
    def _parse_rss(
        ...
        try:
            ...
            for item in items:
                try:
                    article = await self._parse_item(item, feed_url, feed_title, root.tag)
    ...
    async def _parse_item(
        ...
        if original_url:
            try:
                ...
                if article_content:
                    logger.debug(f"Fetched {len(article_content)} chars of article content from {original_url}")
                else:
                    logger.warning(f"Could not fetch article content from {original_url}")
            except Exception as e:
                logger.warning(f"Error fetching article content from {original_url}: {e}")
