...
Choose the most appropriate category. Respond with only the category name."""
    ...
    async def generate_commentary(self, article_content: str, user_highlights: str, article_title: str = "") -> str:
...
Write the commentary:"""
    ...
    async def editorial_roast(self, content: str, content_type: str = "article") -> Dict[str, Any]:
...
Format: SCORE: X/10\nFEEDBACK: [your comprehensive roast]\nAPPROVED: YES/NO"""
            ...
            if response and "choices" in response and len(response["choices"]) > 0:
                ...
                approved_match = re.search(r'APPROVED:\s*(YES|NO)', review, re.IGNORECASE)
                feedback_match = re.search(r'FEEDBACK:\s*(.+?)(?=APPROVED:|$)', review, re.DOTALL)
                
                score = int(score_match.group(1)) if score_match else 5
                approved = approved_match.group(1).upper() == "YES" if approved_match else score >= 7
    ...
    async def revise_content(self, original_content: str, editor_feedback: str, article_content: str = "", user_highlights: str = "") -> str:
