...
async def debug_single_article():
    ...
    print(
        f"🤖 OpenRouter API key: {openrouter_key[:20]}..."
        if openrouter_key
        else "Not set"
    )
    ...
    if not review["approved"]:
        ...
        revised_commentary = await openrouter_client.revise_content(
            commentary, review["feedback"], article_content, item.content
        )
        print(f"   Revised commentary: {len(revised_commentary)} characters")

        # Second review
        print(f"👨‍💼 Step 5: Second editorial review...")
        final_review = await openrouter_client.editorial_roast(
            revised_commentary, "article"
        )
